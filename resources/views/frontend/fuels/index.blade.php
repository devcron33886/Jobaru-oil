        ...
        <div class="col-md-12">
            @can('fuel_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.fuels.create') }}">
                            ...
                            <tbody>
                                @foreach($fuels as $key => $fuel)
                                    <tr data-entry-id="{{ $fuel->id }}">
                                        ...
                                        <td>
                                            @can('fuel_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.fuels.show', $fuel->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('fuel_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.fuels.edit', $fuel->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('fuel_delete')
                                                <form action="{{ route('frontend.fuels.destroy', $fuel->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('fuel_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.fuels.massDestroy') }}",
